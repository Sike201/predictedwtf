import { PublicKey, Transaction, ComputeBudgetProgram, Keypair, SystemProgram, SYSVAR_RENT_PUBKEY, TransactionInstruction } from '@solana/web3.js';
import { getMint, getAssociatedTokenAddressSync, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, createBurnInstruction, createTransferInstruction, createMintToInstruction, getAccount, createAssociatedTokenAccountIdempotentInstruction, getAssociatedTokenAddress, NATIVE_MINT, createAssociatedTokenAccountInstruction, MINT_SIZE } from '@solana/spl-token';
...
function loadMarketEngineAuthority() {
  ...
  try {
    ...
    return Keypair.fromSecretKey(import_bs58.default.decode(raw));
  } catch {
    return null;
  }
}
...
async function buildWithdrawOmnipairLiquidityToUsdcTransactionEngineSigned(params) {
  ...
  const [userYesBefore, userNoBefore, lpDecimalsResolved] = await Promise.all([
    ...
    params.lpDecimals != null ? Promise.resolve(params.lpDecimals) : getMint(params.connection, pairDecoded.lpMint).then(
      (m) => m.decimals
    )
  ]);
...
export { PredictedClient, runBuyOutcome, runCreateOmnipairMarket, runDepositLiquidity, runResolveMarketRedemption, runSellOutcome, runWithdrawLiquidityToUsdc, solanaTransactionExplorerUrl, toPublicKey };
